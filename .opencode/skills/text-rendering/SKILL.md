...
### 规则 1 ─ 文字内容必须**逐字精确**用直角引号包起来

```
✅ 好：The poster shows the headline "梦回水乡" in seal-script Chinese typography.
❌ 差：The poster shows a Chinese title meaning "dream of water town".
```

模型对**精确字符**的渲染稳定性远高于"含义描述"。直角引号 `「」` 或英文 `"…"` 都可以。
...
### 规则 6 ─ Negative Prompt 要禁止"乱码字"而不是禁止"任何字"

旧 prompt 里的 `no embedded text / no AI hallucinated text` 现在要改成：

```
✅ 好：Negative: no garbled characters, no Western letters mistaken for Chinese, no missing strokes, no extra strokes.
...
[STYLE_DIRECTIVE].
[BRAND_DESCRIPTION].
Render the Chinese text "[EMBED_TEXT]" in [FONT_EN_NAME] [WEIGHT], [SIZE_HINT], [LAYOUT_DIRECTION].
[REST_OF_PROMPT_BODY].
Negative: no garbled characters, no Western letters mistaken for Chinese, no missing strokes, no extra strokes, [PLUS_CATEGORY_NEGATIVE].
```
